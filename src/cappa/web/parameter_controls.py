...
class ValueNotSupplied:
    ...
    def __bool__(self: click.types.FuncParamType):
...
class ParameterControls(Widget):
    ...
    def get_values(self) -> ...:
        ...
        controls = list(self.query(f".{self.schema.field_name}"))
        ...
        if len(controls) == 1 and isinstance(controls[0], MultipleChoice):
            ...
            collected_values = list_to_tuples(collected_values, self.schema.num_args)
            breakpoint()
            # return MultiValueParamData.process_cli_option(collected_values)

    def get_control_method(
        self, argument_type: Any
    ) -> Callable[[Any, Text, bool, Arg, str], Widget]:
        text_click_types = {
            click.STRING,
            click.FLOAT,
            click.INT,
