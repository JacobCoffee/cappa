...
def verify_type_compatibility(arg: Arg, field_name: str, annotation: TypeView):
    ...
    if annotation.is_union:
        all_same_arity = {
            ...
            for ta in annotation.inner_types
            if not ta.is_none_type
        }
...
def infer_choices(arg: Arg, annotation: TypeView) -> list[str] | None:
    if arg.choices is None:
        choices = None
        if annotation.is_subclass_of(enum.Enum):
            choices = [v.value for v in annotation.annotation]

        if annotation.is_subclass_of((tuple, list, set)):
            annotation = annotation.inner_types[0]

        if annotation.is_union:
            if all(t.is_literal for t in annotation.inner_types):
                choices = [typing.get_args(t)[0] for t in annotation.args]

        if annotation.is_literal:
            choices = list(annotation.args)
...
def infer_action(
    ...
    annotation = annotation.unwrap_optional()
    ...
    if annotation.is_unbounded_tuple:
        return ArgAction.append
...
def infer_num_args(
    ...
    if annotation.is_tuple and not annotation.is_unbounded_tuple:
        return len(annotation.args)

    if annotation.is_unbounded_tuple and is_positional:
        return -1
    return 1
