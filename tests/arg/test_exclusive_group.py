...
@backends
def test_implicit_union_syntax(backend, capsys):
    @dataclass
    class ArgTest:
        wat: (
            Annotated[int, cappa.Arg(short="-v")]
            | Annotated[str, cappa.Arg(short="-p")]
        )

    with pytest.raises(cappa.Exit):
        parse(ArgTest, "-v", "1", "-p", "2", backend=backend)

    result = parse(ArgTest, "-v", "1", backend=backend)
    assert result.wat == 1

    result = parse(ArgTest, "-p", "1", backend=backend)
    assert result.wat == "1"
